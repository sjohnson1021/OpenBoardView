...
void XZZPCBFile::process_block(uint8_t block_type, std::vector<char> &block_buf) {
	switch (block_type) {
		...
		case 0x02: { // VIA
			...
			std::vector<uint32_t> via_data((uint32_t *)(block_buf.data()), (uint32_t *)(block_buf.data() + block_buf.size()));
			break;
		}
...
void XZZPCBFile::assign_component_sides() {
    for (auto &part : parts) {
        ...
        if (pin_count > 0) {
            // Handle single-pin components (likely drill holes or test pads)
            if (pin_count == 1 && start_pin < pins.size()) {
                auto &pin = pins[start_pin];
                
                // Check if this is a drill/screw hole (single pin on GND or unconnected)
                if (strcmp(pin.net, "GND") == 0 || 
                    strcmp(pin.net, "GROUND") == 0 || 
                    strcmp(pin.net, "UNCONNECTED") == 0 || 
                    strcmp(pin.net, "NC") == 0 || 
                    strlen(pin.net) == 0) {
                    
                    // Prefix name with "..." to identify as special
                    if (strncmp(part.name, "...", 3) != 0) {
                        // Only add prefix if not already present
                        char * new_name = (char *)malloc(strlen(part.name) + 4);
                        sprintf(new_name, "...%s", part.name);
                        free((void*)part.name);
                        part.name = new_name;
                    }
                    ...
                    // We'll let BRDBoard determine whether it's a test pad or drill hole
                    // based on the net name and these markers we've set
                    continue;
                }
            }
...
void XZZPCBFile::parse_test_pad_block(std::vector<uint8_t> &buf) {
	...
	current_pointer += 4;
	uint32_t inner_diameter = *reinterpret_cast<uint32_t *>(&buf[current_pointer]);
	current_pointer += 8; // + unknown1
